...
(()=>{'use strict';
 ...
 function boot(){mobileNavReels();socialShortcut();if(page==='explore')explore();if(page==='messages')messages();if(page==='profile')profile();if(page==='create-listing')createSteps();if(page==='auth')oauth();realtimeSounds()}
 addEventListener('avtovip:language',()=>{ /* app.js + social.js own the full dynamic translation cycle */ });
 if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot();addEventListener('resize',()=>{clearTimeout(window.__avNavResize);window.__avNavResize=setTimeout(mobileNavReels,120)},{passive:true});
})();
