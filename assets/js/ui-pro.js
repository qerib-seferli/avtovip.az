...
(()=>{'use strict';
 ...
 function explore(){const search=$('.explore-search');if(!search||$('.av-explore-tools'))return;const tools=document.createElement('section');tools.className='av-explore-tools';tools.innerHTML=`<div class="av-chip-rail"><button class="av-chip active">${t('all')}</button><button class="av-chip">Minik</button><button class="av-chip">SUV</button><button class="av-chip">Krossover</button><button class="av-chip">Yük</button><button class="av-chip">Motosiklet</button></div><div><div class="av-mini-title"><span>🔥 ${t('popular')}</span><a href="index.html#axtaris">${t('all')} ›</a></div><div class="av-chip-rail"><button class="av-chip" data-q="Audi">Audi</button><button class="av-chip" data-q="BMW">BMW</button><button class="av-chip" data-q="Mercedes-Benz">Mercedes</button><button class="av-chip" data-q="Toyota">Toyota</button><button class="av-chip" data-q="Hyundai">Hyundai</button><button class="av-chip" data-q="Kia">Kia</button></div></div></div>`;search.after(tools);tools.addEventListener('click',e=>{const b=e.target.closest('[data-q]');if(!b)return;const i=$('#exploreSearch');i.value=b.dataset.q;i.dispatchEvent(new Event('input',{bubbles:true}));i.focus()});}
 function messages(){const list=$('#conversationList');if(!list||$('.av-message-tools'))return;const tools=document.createElement('div');tools.className='av-message-tools';tools.innerHTML=`<button class="active">${t('all')}</button><button>${t('unread')}</button><button>${t('buyers')}</button><button>${t('sellers')}</button>`;list.before(tools);const search=document.createElement('div');search.className='explore-search';search.style.margin='0 0 10px';search.innerHTML=`<i class="fa-solid fa-magnifying-glass"></i><input id="avMessageSearch" type="search" placeholder="${t('searchMsg')}">`;tools.after(search);search.querySelector('input').addEventListener('input',e=>{const q=e.target.value.toLocaleLowerCase();$$('.conversation-item',list).forEach(x=>x.hidden=q&&!x.textContent.toLocaleLowerCase().includes(q))});}
