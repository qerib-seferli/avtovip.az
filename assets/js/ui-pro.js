...
(()=>{'use strict';
 ...
 function messages(){const host=$('#avMessageToolsHost'),searchHost=$('#avMessageSearchHost'),list=$('#conversationList');if(!list||!host||host.dataset.ready==='1')return;host.dataset.ready='1';host.className='av-message-tools';host.innerHTML=`<button class="active" data-message-filter="all">${t('all')}</button><button data-message-filter="unread">${t('unread')}</button><button data-message-filter="buyers">${t('buyers')}</button><button data-message-filter="sellers">${t('sellers')}</button>`;window.__avMessageFilter=window.__avMessageFilter||'all';host.addEventListener('click',e=>{const b=e.target.closest('[data-message-filter]');if(!b)return;window.__avMessageFilter=b.dataset.messageFilter;host.querySelectorAll('[data-message-filter]').forEach(x=>x.classList.toggle('active',x===b));window.__avApplyMessageSearch?.()});searchHost.className='explore-search av-message-search';searchHost.innerHTML=`<i class="fa-solid fa-magnifying-glass"></i><input id="avMessageSearch" type="text" inputmode="search" autocomplete="off" placeholder="${t('searchMsg')}"><button id="avMessageSearchClear" class="av-message-search-clear" type="button" aria-label="Təmizlə" hidden><i class="fa-solid fa-xmark"></i></button>`;const input=searchHost.querySelector('input'),clear=searchHost.querySelector('#avMessageSearchClear');const apply=()=>{window.__avMessageSearchQuery=input.value||'';clear.hidden=!input.value;window.__avApplyMessageSearch?.()};input.addEventListener('input',apply);clear.addEventListener('click',()=>{input.value='';apply();input.focus({preventScroll:true})});
 if(window.visualViewport&&!window.__avMessageViewportReady){window.__avMessageViewportReady=true;const syncViewport=()=>{const vv=window.visualViewport;document.documentElement.style.setProperty('--av-visual-height',`${Math.round(vv.height)}px`);const keyboardOpen=(window.innerHeight-vv.height)>120;document.documentElement.classList.toggle('av-keyboard-open',keyboardOpen);if(keyboardOpen&&document.activeElement?.matches?.('#chatInput,#avMessageSearch'))setTimeout(()=>document.activeElement?.scrollIntoView?.({block:'nearest',behavior:'smooth'}),30)};window.visualViewport.addEventListener('resize',syncViewport,{passive:true});window.visualViewport.addEventListener('scroll',syncViewport,{passive:true});window.addEventListener('orientationchange',()=>setTimeout(syncViewport,100),{passive:true});syncViewport()}
 document.addEventListener('focusin',e=>{if(e.target?.matches?.('#chatInput,#avMessageSearch'))setTimeout(()=>e.target.scrollIntoView({block:'nearest',behavior:'smooth'}),180)});}
 async function profile(){const head=$('.profile-head');if(!head||$('.av-profile-actions'))return;const stats=document.createElement('div');stats.className='av-profile-stats';stats.innerHTML=`<div><b id="avFollowers">0</b><span>Takipçi</span></div><div><b id="avFollowing">0</b><span>Takip</span></div><div><b id="avListings">0</b><span>${t('ads')}</span></div><div><b id="avLikes">0</b><span>${t('likes')}</span></div>`;head.append(stats);const actions=document.createElement('div');actions.className='av-profile-actions';actions.innerHTML=`<button data-open-profile><i class="fa-regular fa-user"></i><span>${t('profileInfo')}</span></button><a href="ayarlar.html"><i class="fa-solid fa-gear"></i><span>${t('settings')}</span></a><a href="sevimliler.html"><i class="fa-regular fa-heart"></i><span>${t('saved')}</span></a><a href="odenisler.html"><i class="fa-regular fa-clock"></i><span>${t('history')}</span></a>`;head.after(actions);actions.querySelector('[data-open-profile]').onclick=()=>$('#profileInfoToggle')?.click();try{const cur=await window.avtoDb.current();if(cur.profile?.role==='admin'&&!actions.querySelector('[data-admin-panel]')){const admin=document.createElement('a');admin.href='admin/index.html';admin.dataset.adminPanel='1';admin.className='av-admin-panel-entry';admin.innerHTML='<i class="fa-solid fa-shield-halved"></i><span>Admin panel</span>';actions.append(admin);actions.classList.add('has-admin')}const {data:{session}}=await sb.auth.getSession();const id=session?.user?.id;if(!id)return;const [a,b,c,d]=await Promise.all([sb.from('user_follows').select('*',{count:'exact',head:true}).eq('following_id',id),sb.from('user_follows').select('*',{count:'exact',head:true}).eq('follower_id',id),sb.from('elanlar').select('*',{count:'exact',head:true}).eq('user_id',id),sb.from('listing_likes').select('*',{count:'exact',head:true}).eq('user_id',id)]);$('#avFollowers').textContent=a.count||0;$('#avFollowing').textContent=b.count||0;$('#avListings').textContent=c.count||0;$('#avLikes').textContent=d.count||0}catch{}}
 ...
 function socialShortcut(){const a=$('.header-actions');if(!a||$('#avSocialShortcut')||['auth','reset','feed','reels'].includes(page))return;const x=document.createElement('a');x.id='avSocialShortcut';x.className='icon-btn';x.href='lent.html';x.setAttribute('aria-label','Avto Lent');x.innerHTML='<i class="fa-solid fa-bolt"></i>';a.insertBefore(x,a.firstChild)}
 function mobileSocialCross(){
   const a=$('.header-actions');if(!a)return;const old=$('.av-mobile-social-cross');old?.remove();
   if(!matchMedia('(max-width:900px)').matches||!['feed','reels'].includes(page))return;
   const x=document.createElement('a');x.className='icon-btn av-mobile-social-cross';
   if(page==='reels'){x.href='lent.html';x.setAttribute('aria-label','Avto Lent');x.innerHTML='<i class="fa-solid fa-bolt"></i>'}
   else{x.href='reels.html';x.setAttribute('aria-label','Reels');x.innerHTML='<i class="fa-solid fa-clapperboard"></i>'}
 ...
 function mobileNavReels(){
   ...
   if(mobile){if(!profileLink.dataset.avOriginalHref){profileLink.dataset.avOriginalHref=profileLink.getAttribute('href')||'profile.html';profileLink.dataset.avOriginalHtml=profileLink.innerHTML}profileLink.href='reels.html';profileLink.dataset.nav='reels';profileLink.innerHTML='<i class="fa-solid fa-clapperboard"></i><span>Reels</span>';profileLink.classList.toggle('active',page==='reels')}
