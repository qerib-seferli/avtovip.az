...
(() => {
  ...
  async function openStory(s,p={}){
    ...
    const loadSocial=async()=>{
      ...
      commentsRoot.innerHTML=(comments||[]).length?(comments||[]).map(c=>{const u=users[c.user_id]||{},own=c.user_id===currentUser?.id;return `<div class="story-comment" data-story-comment="${c.id}"><img src="${esc(u.avatar_url||'assets/img/brand/icon-192.png')}" alt=""><div><div class="story-comment-meta"><strong>${esc([u.name,u.surname].filter(Boolean).join(' ')||'İstifadəçi')}</strong><time>${dateText(c.created_at)}${c.edited_at?' · ✎':''}</time></div><p>${esc(c.body)}</p>${own?`<div class="story-comment-actions"><button type="button" data-story-edit="${c.id}">Redaktə et</button><button type="button" data-story-delete="${c.id}">Sil</button></div>`:''}</div></div>`}).join(''):'<div class="story-social-empty">Hələ şərh yoxdur. İlk şərhi siz yazın.</div>';
    };
