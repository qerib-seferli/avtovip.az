...
(() => {
  ...
  async function openStory(s,p={},storyList=[],storyProfiles={}){
    ...
    const ownerVerified=p?.is_verified?'<span class="verified-mark av-identity-verified story-verified" title="Təsdiqlənmiş hesab"></span>':'';
  ...
  async function initProfile(){
    ...
    $('#profileName').innerHTML=`${profile?.is_verified?'<span class="verified-mark av-identity-verified profile-verified" title="Təsdiqlənmiş hesab"></span>':''}${flag?`<span class="profile-country-flag" data-country="${esc(country)}" aria-label="${esc(country)}">${esc(flag)}</span>`:''}<span class="profile-name-label">${esc([profile?.name,profile?.surname].filter(Boolean).join(' ')||user.email)}</span>`;
