...
(() => {
  ...
  function listingBadges(x){
    ...
    if(x.is_credit)out+='<span class="badge listing-icon-badge" title="Kredit" aria-label="Kredit"><i class="fa-solid fa-hand-holding-dollar"></i></span>';
    if(x.is_barter)out+='<span class="badge listing-icon-badge" title="Barter" aria-label="Barter"><i class="fa-solid fa-right-left"></i></span>';
    return out;
  }
  ...
  function renderDetail(x,seller,hist,insight,isFav){
    ...
    const detailSpecs=[
      ...
      ['Oturacaq sayı',x.seats??'-'],['Sahib sayı',x.owners_count??'-'],['Bazar mənşəyi',x.market_origin||'-'],['VIN',x.vin||'-'],
      ['Ölkə',`${countryFlag(x.country_code)} ${x.country_code||'-'}`.trim()],['Region / Ştat',x.state_name||'-'],['Şəhər',x.city||'-'],['Rayon / Bölgə',x.district||'-'],
      ['Kredit',x.is_credit?'Bəli':'Xeyr'],['Barter',x.is_barter?'Bəli':'Xeyr'],['Vuruğu var',x.has_accident?'Bəli':'Xeyr'],['Rənglənib',x.is_painted?'Bəli':'Xeyr']
    ];
    root.innerHTML=`<div class="detail-layout"><div class="stack"><section class="panel gallery"><div class="gallery-main" id="galleryMain"><img id="mainPhoto" src="${esc(photos[0])}" alt="${esc(x.title||`${x.brand} ${x.model}`)}"><button class="gallery-nav gallery-prev" type="button" aria-label="Əvvəlki şəkil"><i class="fa-solid fa-chevron-left"></i></button><button class="gallery-nav gallery-next" type="button" aria-label="Növbəti şəkil"><i class="fa-solid fa-chevron-right"></i></button><div class="gallery-zoom-tools"><button type="button" data-gallery-zoom="out" aria-label="Kiçilt"><i class="fa-solid fa-minus"></i></button><button type="button" data-gallery-zoom="reset" aria-label="Ölçünü sıfırla"><i class="fa-solid fa-expand"></i></button><button type="button" data-gallery-zoom="in" aria-label="Böyüt"><i class="fa-solid fa-plus"></i></button></div></div><div class="gallery-thumbs">${photos.map((p,i)=>`<button class="gallery-thumb ${i===0?'active':''}" data-photo="${esc(p)}" data-index="${i}"><img src="${esc(p)}" alt=""></button>`).join('')}</div></section>
    ...
    gallery?.querySelector('[data-gallery-zoom="reset"]')?.addEventListener('click',e=>{e.stopPropagation();resetGalleryZoom()});
    ...
    $('#detailFav').onclick=e=>toggleFavorite(x.id,e.currentTarget); $('#detailCompare').onclick=()=>toggleCompare(x.id); $('#detailShare').onclick=async()=>{try{if(navigator.share)await navigator.share({title:`${x.brand} ${x.model}`,url:location.href});else{await navigator.clipboard.writeText(location.href);toast('Link kopyalandı.','success')}}catch{}}; $('#reportBtn').onclick=()=>reportListing(x.id);
  }
