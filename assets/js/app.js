...
(() => {
  ...
  function renderDetail(x,seller,hist,insight,isFav){
    ...
    const lowerActions=`${x.is_credit?'<span class="gallery-status-action credit" title="Kredit"><i class="fa-solid fa-hand-holding-dollar"></i><span>Kredit</span></span>':''}${x.is_barter?'<span class="gallery-status-action barter" title="Barter"><i class="fa-solid fa-right-left"></i><span>Barter</span></span>':''}<button type="button" class="gallery-action-btn" id="detailPhotoShare" title="Paylaş"><i class="fa-solid fa-share-nodes"></i><span>Paylaş</span></button><button type="button" class="gallery-action-btn ${isFav?'active':''}" id="detailPhotoSave" title="Kaydet"><i class="fa-${isFav?'solid':'regular'} fa-bookmark"></i><span>Kaydet</span></button>`;
    ...
    const syncDetailFavorite=()=>{const active=$('#detailPhotoFav')?.classList.contains('active');const heart=$('#detailPhotoFav'),save=$('#detailPhotoSave');if(heart){heart.classList.toggle('active',!!active);heart.innerHTML=`<i class="fa-${active?'solid':'regular'} fa-heart"></i>`}if(save){save.classList.toggle('active',!!active);save.innerHTML=`<i class="fa-${active?'solid':'regular'} fa-bookmark"></i><span>Kaydet</span>`}};
    ...
    $('#detailPhotoFav')?.addEventListener('click',toggleDetailFavorite);$('#detailPhotoSave')?.addEventListener('click',toggleDetailFavorite);
    $('#detailPhotoShare')?.addEventListener('click',shareListing);
  ...
  function initPWA(){
    if('serviceWorker' in navigator){
      window.addEventListener('load',async()=>{
        try{
          const reg=await navigator.serviceWorker.register('./service-worker.js?v=33',{scope:'./',updateViaCache:'none'});
          await reg.update();
