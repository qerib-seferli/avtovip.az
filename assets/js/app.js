...
(() => {
  ...
  async function renderThread(peer,listing=''){
    ...
    root.onclick=async e=>{const del=e.target.closest('[data-delete-message]'),edit=e.target.closest('[data-edit-message]');if(del){if(!confirm('Bu mesaj silinsin?'))return;const {error}=await sb.from('messages').update({deleted_at:new Date().toISOString(),body:'',media_url:null,media_type:null}).eq('id',del.dataset.deleteMessage).eq('sender_id',currentUser.id);if(error)toast(error.message,'error');else renderThread(peer,listing)}if(edit){const id=edit.dataset.editMessage,b=edit.closest('.bubble'),old=b.querySelector('p')?.textContent||'';const text=prompt('Mesajı redaktə edin:',old);if(text===null)return;const {error}=await sb.from('messages').update({body:text.trim(),edited_at:new Date().toISOString()}).eq('id',id).eq('sender_id',currentUser.id);if(error)toast(error.message,'error');else renderThread(peer,listing)}};
