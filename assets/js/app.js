...
(() => {
  ...
  async function openStory(s,p={}){
    ...
    const video=modal.querySelector('video');if(video){video.muted=false;video.volume=1;video.play().catch(()=>{video.autoplay=false})}else setTimeout(()=>modal.isConnected&&close(),7000);
    let key=localStorage.getItem('avtovip-viewer-key');if(!key){key=crypto.randomUUID?.()||Math.random().toString(36).slice(2);localStorage.setItem('avtovip-viewer-key',key)} sb.from('story_views').insert({story_id:s.id,viewer_id:currentUser?.id||null,viewer_key:currentUser?'':key}).then(()=>{});
    ...
    modal.querySelector('#storyCommentFocus').onclick=()=>commentInput.focus();
    ...
    modal.querySelector('#storyCommentForm').onsubmit=async e=>{e.preventDefault();if(!currentUser){toast('Şərh yazmaq üçün giriş edin.','info');return}const body=commentInput.value.trim();if(!body)return;const {error}=await sb.from('story_comments').insert({story_id:s.id,user_id:currentUser.id,body});if(error)toast(error.message,'error');else{commentInput.value='';await loadSocial();commentsRoot.scrollTop=commentsRoot.scrollHeight}};
