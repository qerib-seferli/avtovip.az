...
(() => {
  ...
  function initThemeLang(){
    ...
    if(!langSel && ['auth','reset'].includes(page)){
  ...
  function brandLogoCandidates(make){const name=String(make?.name||'');const local=LOCAL_BRAND_LOGOS[name.toLowerCase()]||'';const snap=typeof make?.logo==='string'?make.logo:(make?.logo?.local_url||'');const remote=typeof make?.logo==='object'?(make?.logo?.url||''):'';const slugMap={'Mercedes-Benz':'mercedes-benz','Lada/VAZ':'lada','Land Rover':'land-rover','Alfa Romeo':'alfa-romeo','Aston Martin':'aston-martin','AM General':'am-general','Rolls-Royce':'rolls-royce'};const logoSlug=(slugMap[name]||name.toLowerCase().normalize('NFKD').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,''));const curatedSvg=logoSlug?`https://cdn.jsdelivr.net/gh/vehiclespecs/brand-logos@main/${logoSlug}-logo.svg`:'';const curatedPng=logoSlug?`https://cdn.jsdelivr.net/gh/vehiclespecs/brand-logos@main/${logoSlug}-logo.png`:'';const simpleSlug=name.toLowerCase().normalize('NFKD').replace(/[^a-z0-9]/g,'');const simple=simpleSlug?`https://cdn.simpleicons.org/${simpleSlug}`:'';return [...new Set([local,snap,remote,curatedSvg,curatedPng,simple].filter(Boolean))];}
  ...
  async function blockedRelationship(peer){
    if(!currentUser||!peer)return false;
    const a=await sb.from('user_blocks').select('blocker_id').or(`and(blocker_id.eq.${currentUser.id},blocked_id.eq.${peer}),and(blocker_id.eq.${peer},blocked_id.eq.${currentUser.id})`).limit(1);
    return !!(a.data&&a.data.length);
  }
  async function openConversation(peer,listing=''){$('#messagesShell').classList.add('chat-selected');const pane=$('#chatPane');pane.classList.add('open');pane.dataset.peer=peer;pane.dataset.listing=listing||'';const {data:p}=await sb.from('users').select('*').eq('id',peer).maybeSingle();$('#chatPeerName').textContent=[p?.name,p?.surname].filter(Boolean).join(' ')||staticText('İstifadəçi');$('#chatPeerAvatar').src=p?.avatar_url||'assets/img/brand/icon-192.png';$('#chatBack')?.addEventListener('click',()=>$('#messagesShell').classList.remove('chat-selected'),{once:true});await renderThread(peer,listing);const form=$('#chatForm');const blocked=await blockedRelationship(peer);form?.classList.toggle('chat-blocked',blocked);const input=$('#chatInput');const send=form?.querySelector('button[type="submit"]');if(input){input.disabled=blocked;input.placeholder=blocked?runtimeText('Bu istifadəçi ilə mesajlaşma mümkün deyil.'):runtimeText('Mesaj yazın...')}if(send)send.disabled=blocked;if(blocked){const root=$('#chatMessages');if(root&&!root.querySelector('.blocked-chat-note'))root.insertAdjacentHTML('beforeend',`<div class="blocked-chat-note">${esc(runtimeText('Bu istifadəçi ilə mesajlaşma mümkün deyil.'))}</div>`)}form.onsubmit=async e=>{e.preventDefault();if(await blockedRelationship(peer)){toast(runtimeText('Bu istifadəçi ilə mesajlaşma mümkün deyil.'),'error');return}const input=$('#chatInput');const body=input.value.trim();if(!body)return;input.value='';const {error}=await sb.from('messages').insert({listing_id:listing||null,sender_id:currentUser.id,receiver_id:peer,body});if(error){toast(error.message==='AVTOVIP_BLOCKED_MESSAGE'?runtimeText('Bu istifadəçi ilə mesajlaşma mümkün deyil.'):error.message,'error');input.value=body}else renderThread(peer,listing)}}
