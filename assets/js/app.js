...
(() => {
  ...
  async function renderThread(peer,listing=''){
    ...
    root.onclick=async e=>{const del=e.target.closest('[data-delete-message]'),edit=e.target.closest('[data-edit-message]');if(del){const ok=await avMessageDialog({mode:'confirm',title:runtimeText('Mesajı sil'),text:runtimeText('Bu mesaj silinsin?'),confirmText:runtimeText('Sil')});if(!ok)return;const id=del.dataset.deleteMessage,b=del.closest('.bubble');const {error}=await sb.from('messages').update({deleted_at:new Date().toISOString(),body:'',media_url:null,media_type:null}).eq('id',id).eq('sender_id',currentUser.id);if(error)toast(error.message,'error');else{b?.remove();renderConversations(currentUser.id)}}if(edit){const id=edit.dataset.editMessage,b=edit.closest('.bubble'),old=b.querySelector('p')?.textContent||'';const text=await avMessageDialog({mode:'prompt',title:runtimeText('Mesajı redaktə edin'),value:old,confirmText:runtimeText('Yadda saxla')});if(text===null)return;const clean=text.trim();if(!clean)return;const {error}=await sb.from('messages').update({body:clean,edited_at:new Date().toISOString()}).eq('id',id).eq('sender_id',currentUser.id);if(error)toast(error.message,'error');else{const para=b?.querySelector('p');if(para)para.textContent=clean;const tm=b?.querySelector('time');if(tm&&!tm.textContent.includes('✎'))tm.textContent+=' · ✎';renderConversations(currentUser.id)}}};
