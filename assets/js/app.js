...
(() => {
  ...
  async function initCreateListing(){
    ...
    if(editId){
      ...
      set('brand',data.brand);$('#brand')?.dispatchEvent(new Event('change',{bubbles:true}));await new Promise(r=>setTimeout(r,0));
      ['model','generation','trim','state_name','city','district','price','year','mileage','engine_volume','engine_power','seats','owners_count','market_origin','vin','description','phone','whatsapp_phone'].forEach(k=>set(k,data[k]));
      ['currency','country_code','body_type','color','fuel','transmission','drivetrain'].forEach(k=>set(k,data[k]));
      set('condition',data.is_new?'new':'used');
      ['is_credit','is_barter','has_accident','is_painted'].forEach(k=>{const el=form.elements.namedItem(k);if(el)el.checked=!!data[k]});
      if(Array.isArray(data.equipment))for(const cb of form.querySelectorAll('input[name="equipment"]'))cb.checked=data.equipment.includes(cb.value);
      drawEditPreview();
