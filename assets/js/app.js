...
(() => {
  ...
  const IDENTITY_BADGE_ASSETS = {
    16:'assets/img/identity/mavi-tik-16x16.svg',
    24:'assets/img/identity/mavi-tik-24x24.svg',
    32:'assets/img/identity/mavi-tik-32x32.svg',
    48:'assets/img/identity/mavi-tik-48x48.svg',
    64:'assets/img/identity/mavi-tik-64x64.svg'
  };
  const verifiedBadge=(size=16,extraClass='')=>{
    const badgeSize=IDENTITY_BADGE_ASSETS[size]?size:16;
    return `<span class="av-user-verified av-user-verified--${badgeSize}${extraClass?` ${extraClass}`:''}" title="Təsdiqlənmiş hesab" aria-label="Təsdiqlənmiş hesab"><img src="${IDENTITY_BADGE_ASSETS[badgeSize]}" alt="" aria-hidden="true"></span>`;
  };
  const avatarVerifiedClass=isVerified=>isVerified?' is-verified':'';
  const tierIcon=tier=>String(tier||'').toLowerCase()==='vip'?'<i class="fa-solid fa-crown avatar-crown" aria-label="VIP"></i>':String(tier||'').toLowerCase()==='premium'?'<i class="fa-solid fa-gem avatar-gem" aria-label="Premium"></i>':'';
  window.AvtoVIPIdentity = Object.assign({}, window.AvtoVIPIdentity, { verifiedBadge, avatarVerifiedClass, tierIcon, badgeAssets: IDENTITY_BADGE_ASSETS });
  ...
  async function openStory(s,p={},storyList=[],storyProfiles={}){
    ...
    const ownerAvatar=`<span class="avatar-shell story-owner-avatar ${ownerTier}${avatarVerifiedClass(p?.is_verified)}"><img src="${esc(p?.avatar_url||'assets/img/brand/icon-192.png')}" alt="">${ownerTierMark}</span>`;
    ...
    const loadSocial=async()=>{
      ...
      commentsRoot.innerHTML=(comments||[]).length?(comments||[]).map(c=>{const u=users[c.user_id]||{},own=c.user_id===currentUser?.id,canDelete=!!currentUser&&(own||s.user_id===currentUser.id),profileUrl=`user.html?id=${encodeURIComponent(c.user_id)}`,tier=u.membership_tier||'free',tierMark=tier==='vip'?'<i class="fa-solid fa-crown avatar-crown story-avatar-tier" aria-label="VIP"></i>':tier==='premium'?'<i class="fa-solid fa-gem avatar-gem story-avatar-tier" aria-label="Premium"></i>':'';return `<div class="story-comment" data-story-comment="${c.id}"><a class="story-comment-avatar avatar-shell ${tier}${avatarVerifiedClass(u.is_verified)}" href="${profileUrl}"><img src="${esc(u.avatar_url||'assets/img/brand/icon-192.png')}" alt="">${tierMark}</a><div><div class="story-comment-meta"><a class="story-comment-user" href="${profileUrl}"><strong>${u.is_verified?verifiedBadge():''}<span class="story-comment-name-text">${esc([u.name,u.surname].filter(Boolean).join(' ')||'İstifadəçi')}</span></strong></a><time>${new Intl.DateTimeFormat('az-AZ',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit',hour12:false}).format(new Date(c.created_at))}${c.edited_at?' · ✎':''}</time></div><p>${esc(c.body)}</p>${(own||canDelete)?`<div class="story-comment-actions">${own?`<button type="button" data-story-edit="${c.id}">Redaktə et</button>`:''}${canDelete?`<button type="button" data-story-delete="${c.id}">Sil</button>`:''}</div>`:''}</div></div>`}).join(''):'<div class="story-social-empty">Hələ şərh yoxdur. İlk şərhi siz yazın.</div>';
    };
    ...
    commentsRoot.onclick=async e=>{const del=e.target.closest('[data-story-delete]'),edit=e.target.closest('[data-story-edit]');if(del){const ok=await uiDialog({title:'Şərhi sil',message:'Bu şərh silinsin?',confirmText:'Sil'});if(!ok)return;let q=sb.from('story_comments').delete().eq('id',del.dataset.storyDelete);if(s.user_id!==currentUser?.id)q=q.eq('user_id',currentUser?.id||'');const {error}=await q;if(error)toast(error.message,'error');else await loadSocial()}if(edit){const row=edit.closest('.story-comment'),old=row?.querySelector('p')?.textContent||'';const text=await uiDialog({title:'Şərhi redaktə et',message:'Şərhi dəyişin:',input:true,inputValue:old,confirmText:'Yadda saxla'});if(text===null||!String(text).trim())return;const {error}=await sb.from('story_comments').update({body:String(text).trim(),edited_at:new Date().toISOString()}).eq('id',edit.dataset.storyEdit).eq('user_id',currentUser.id);if(error)toast(error.message,'error');else await loadSocial()}};
    await loadSocial();
  }
  window.AvtoVIPOpenStory=openStory;
  ...
  function renderDetail(x,seller,hist,insight,isFav){
    ...
    root.innerHTML=`<div class="detail-layout"><div class="stack"><section class="panel gallery"><div class="gallery-main" id="galleryMain"><img id="mainPhoto" src="${esc(photos[0])}" alt="${esc(x.title||`${x.brand} ${x.model}`)}"><div class="gallery-vin"><span class="gallery-country-flag">${esc(flag||x.country_code||'')}</span><span class="gallery-vin-code">${esc(x.vin||'VIN yoxdur')}</span>${x.vin?'<button type="button" id="copyVin" aria-label="VIN kopyala" title="VIN kopyala"><i class="fa-regular fa-copy"></i></button>':''}</div><button type="button" class="gallery-fav ${isFav?'active':''}" id="detailPhotoFav" aria-label="Sevimlilər"><i class="fa-${isFav?'solid':'regular'} fa-heart"></i></button><button class="gallery-nav gallery-prev" type="button" aria-label="Əvvəlki şəkil"><i class="fa-solid fa-chevron-left"></i></button><button class="gallery-nav gallery-next" type="button" aria-label="Növbəti şəkil"><i class="fa-solid fa-chevron-right"></i></button><div class="gallery-photo-actions">${lowerActions}</div><div class="gallery-zoom-tools"><button type="button" data-gallery-zoom="out" aria-label="Kiçilt"><i class="fa-solid fa-minus"></i></button><button type="button" data-gallery-full aria-label="Tam ekran"><i class="fa-solid fa-expand"></i></button><button type="button" data-gallery-zoom="in" aria-label="Böyüt"><i class="fa-solid fa-plus"></i></button></div></div><div class="gallery-thumbs">${photos.map((p,i)=>`<button class="gallery-thumb ${i===0?'active':''}" data-photo="${esc(p)}" data-index="${i}"><img src="${esc(p)}" alt=""></button>`).join('')}</div></section>
      ...
      <aside class="detail-sidebar stack"><section class="panel seller-card"><div class="row"><span class="seller-avatar-shell avatar-shell ${seller?.membership_tier||'free'}${avatarVerifiedClass(seller?.is_verified)}"><img class="seller-avatar" src="${esc(seller?.avatar_url||'assets/img/brand/icon-192.png')}" alt="">${seller?.membership_tier==='vip'?'<i class="fa-solid fa-crown avatar-crown" aria-label="VIP"></i>':seller?.membership_tier==='premium'?'<i class="fa-solid fa-gem avatar-gem" aria-label="Premium"></i>':''}</span><div class="grow"><strong class="seller-name-line">${seller?.is_verified?verifiedBadge():''}<span class="seller-name-text">${esc(name)}</span></strong><div class="muted small">${seller?.role==='dealer'?'Diler':'Şəxsi satıcı'}</div><div class="detail-trust-inline"><span class="detail-trust-stars" aria-label="Etibar ulduzları">${Array.from({length:5},(_,i)=>`<i class="fa-solid fa-star ${i<Math.max(0,Math.min(5,Math.floor(Number(seller?.trust_score??50)/20)))?'earned':''}"></i>`).join('')}</span><div class="detail-trust-circle-wrap"><span class="detail-trust-circle" style="--score:${Math.max(0,Math.min(100,Math.round(Number(seller?.trust_score??50))))};--trust:${Number(seller?.trust_score??50)>=80?'#37d38a':Number(seller?.trust_score??50)>=60?'#e2b83c':Number(seller?.trust_score??50)>=40?'#ff9f43':'#ef4d56'}" aria-label="Etibar ${Math.max(0,Math.min(100,Math.round(Number(seller?.trust_score??50))))}%"><b>${Math.max(0,Math.min(100,Math.round(Number(seller?.trust_score??50))))}%</b></span><span class="detail-trust-copy"><strong>Etibar</strong><small>${Math.max(0,Math.min(100,Math.round(Number(seller?.trust_score??50))))}/100</small></span></div></div></div></div><div class="contact-grid contact-grid-three"><a class="btn contact-call" href="tel:${esc(phone)}"><i class="fa-solid fa-phone"></i>${t('call')}</a>${wa?`<a class="btn btn-whatsapp contact-whatsapp" target="_blank" rel="noopener" href="https://wa.me/${wa}?text=${encodeURIComponent(`Salam. AvtoVİP-da ${x.brand} ${x.model} (${x.year}) elanınızla maraqlanıram: ${location.href}`)}"><i class="fa-brands fa-whatsapp"></i>${t('whatsapp')}</a>`:'<button class="btn btn-outline contact-whatsapp" disabled>WhatsApp</button>'}${currentUser?.id===x.user_id?`<button class="btn btn-outline contact-message" disabled><i class="fa-regular fa-comment-dots"></i>Mesaj</button>`:`<a class="btn contact-message" href="mesajlar.html?with=${x.user_id}&listing=${x.id}"><i class="fa-regular fa-comment-dots"></i>Mesaj</a>`}</div><div class="seller-description"><h3>Açıqlama</h3><p>${esc(x.description||'Açıqlama əlavə edilməyib.')}</p></div></section>${hist.length?`<section class="panel panel-pad sidebar-price-history"><div class="section-head"><h3>Qiymət tarixçəsi</h3></div><div class="price-history">${hist.map(h=>`<div class="price-row"><span>${dateText(h.created_at)}</span><span class="price-history-values">${h.old_price!=null?`<del>${money(h.old_price,h.currency)}</del>`:''}<strong>${money(h.new_price,h.currency)}</strong></span></div>`).join('')}</div></section>`:''}<section class="panel panel-pad"><div class="section-head"><h3>Təhlükəsiz alış</h3></div><p class="muted small">Ödəniş etməzdən əvvəl avtomobili və sənədləri yerində yoxlayın. Şübhəli elanları bizə bildirin.</p><button class="btn btn-outline btn-block" id="reportBtn"><i class="fa-regular fa-flag"></i>${t('report')}</button></section></aside></div>`;
  ...
  async function initCreateListing(){
    ...
    const form=$('#listingForm'),files=$('#listingImages'),preview=$('#imagePreview'),editId=qsParam('edit');
    setupFriendlyFileInput(files,{kind:'images'});
    let selectedListingFiles=[],existingUrls=[],editRow=null,removedExisting=[];
    const drawEditPreview=()=>{
      if(!preview)return;
      const old=existingUrls.map((u,i)=>`<div class="upload-tile av-existing-upload"><img src="${esc(u)}" alt=""><button type="button" class="upload-remove" data-remove-existing="${i}" aria-label="Sil"><i class="fa-solid fa-xmark"></i></button></div>`).join('');
      const fresh=selectedListingFiles.map((f,i)=>`<div class="upload-tile"><img src="${URL.createObjectURL(f)}" alt=""><button type="button" class="upload-remove" data-remove-new="${i}" aria-label="Sil"><i class="fa-solid fa-xmark"></i></button></div>`).join('');
    ...
    form?.addEventListener('submit',async e=>{
      e.preventDefault();const btn=$('#submitListing');btn.disabled=true;setStatus('#listingStatus',editId?'Dəyişikliklər saxlanılır...':'Şəkillər yüklənir...');
      const uploadedPaths=[],newUrls=[];
      try{
        const imageFiles=[...files.files].slice(0,15-existingUrls.length);
        if(!editId&&imageFiles.length<3)throw new Error('Minimum 3 avtomobil şəkli əlavə edin.');
