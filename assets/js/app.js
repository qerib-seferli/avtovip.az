...
(() => {
  ...
  async function openStory(s,p={}){
    ...
    const commentsRoot=modal.querySelector('#storyComments'),likeBtn=modal.querySelector('#storyLikeBtn'),likeCount=modal.querySelector('#storyLikeCount'),commentCount=modal.querySelector('#storyCommentCount'),commentInput=modal.querySelector('#storyCommentInput');
    const loadSocial=async()=>{
      const [{data:likes,error:likeErr},{data:comments,error:commentErr}]=await Promise.all([sb.from('story_likes').select('user_id').eq('story_id',s.id),sb.from('story_comments').select('id,user_id,body,created_at,edited_at').eq('story_id',s.id).order('created_at',{ascending:true}).limit(200)]);
      ...
      likeCount.textContent=(likes||[]).length;commentCount.textContent=(comments||[]).length;const mine=(likes||[]).some(x=>x.user_id===currentUser?.id);likeBtn.classList.toggle('active',mine);likeBtn.querySelector('i').className=`fa-${mine?'solid':'regular'} fa-heart`;
      const ids=[...new Set((comments||[]).map(x=>x.user_id))];let users={};if(ids.length){const {data:u}=await sb.from('users').select('id,name,surname,avatar_url').in('id',ids);(u||[]).forEach(x=>users[x.id]=x)}
      commentsRoot.innerHTML=(comments||[]).length?(comments||[]).map(c=>{const u=users[c.user_id]||{},own=c.user_id===currentUser?.id,profileUrl=`user.html?id=${encodeURIComponent(c.user_id)}`;return `<div class="story-comment" data-story-comment="${c.id}"><a class="story-comment-avatar" href="${profileUrl}"><img src="${esc(u.avatar_url||'assets/img/brand/icon-192.png')}" alt=""></a><div><div class="story-comment-meta"><a class="story-comment-user" href="${profileUrl}"><strong>${esc([u.name,u.surname].filter(Boolean).join(' ')||'İstifadəçi')}</strong></a><time>${dateText(c.created_at)}${c.edited_at?' · ✎':''}</time></div><p>${esc(c.body)}</p>${own?`<div class="story-comment-actions"><button type="button" data-story-edit="${c.id}">Redaktə et</button><button type="button" data-story-delete="${c.id}">Sil</button></div>`:''}</div></div>`}).join(''):'<div class="story-social-empty">Hələ şərh yoxdur. İlk şərhi siz yazın.</div>';
    };
    ...
    modal.querySelector('#storyCommentFocus').onclick=()=>{holdStory();commentInput.focus()};commentInput.addEventListener('focus',holdStory);
    const emojiBtn=modal.querySelector('#storyEmojiBtn'),emojiPicker=modal.querySelector('#storyEmojiPicker'),storyEmojis=['😀','😂','😍','🥰','😎','🤩','👍','👏','🔥','❤️','💯','🎉','😢','😮','🤔','🙏','🚗','🏁'];
    if(emojiBtn&&emojiPicker){emojiPicker.innerHTML=storyEmojis.map(x=>`<button type="button" data-story-emoji="${x}">${x}</button>`).join('');emojiBtn.onclick=()=>{emojiPicker.hidden=!emojiPicker.hidden};emojiPicker.onclick=e=>{const b=e.target.closest('[data-story-emoji]');if(!b)return;const start=commentInput.selectionStart??commentInput.value.length,end=commentInput.selectionEnd??start;commentInput.value=commentInput.value.slice(0,start)+b.dataset.storyEmoji+commentInput.value.slice(end);const pos=start+b.dataset.storyEmoji.length;commentInput.focus();commentInput.setSelectionRange(pos,pos);emojiPicker.hidden=true}}
    modal.querySelector('#storyCommentForm').onsubmit=async e=>{e.preventDefault();if(!currentUser){toast('Şərh yazmaq üçün giriş edin.','info');return}const body=commentInput.value.trim();if(!body)return;const {error}=await sb.from('story_comments').insert({story_id:s.id,user_id:currentUser.id,body});if(error)toast(error.message,'error');else{commentInput.value='';await loadSocial();commentsRoot.scrollTop=commentsRoot.scrollHeight;setTimeout(()=>modal.isConnected&&close(),180)}};
  ...
  async function openConversation(peer,listing=''){
    ...
    const {data:p}=await sb.from('users').select('id,name,surname,avatar_url,membership_tier,is_verified').eq('id',peer).maybeSingle();$('#chatPeerName').textContent=[p?.name,p?.surname].filter(Boolean).join(' ')||staticText('İstifadəçi');const peerSub=$('#chatPeerLink .muted.tiny');if(peerSub)peerSub.textContent=window.__avOnlineUsers?.has?.(peer)?'Onlayn':'Oflayn';$('#chatPeerAvatar').src=p?.avatar_url||'assets/img/brand/icon-192.png';const peerLink=$('#chatPeerLink');if(peerLink)peerLink.href=`user.html?id=${encodeURIComponent(peer)}`;
  ...
  async function renderThread(peer,listing=''){
    const {data:clearRow}=await sb.from('conversation_clears').select('cleared_at').eq('user_id',currentUser.id).eq('peer_id',peer).maybeSingle();let q=sb.from('messages').select('*').or(`and(sender_id.eq.${currentUser.id},receiver_id.eq.${peer}),and(sender_id.eq.${peer},receiver_id.eq.${currentUser.id})`).is('deleted_at',null).order('created_at',{ascending:true}).limit(500);if(clearRow?.cleared_at)q=q.gt('created_at',clearRow.cleared_at); if(listing)q=q.eq('listing_id',listing);const {data,error}=await q;const root=$('#chatMessages');if(error){root.innerHTML=esc(error.message);return}root.innerHTML=(data||[]).map(m=>`<div class="bubble ${m.sender_id===currentUser.id?'mine':''}" data-message-id="${m.id}" data-media-url="${esc(m.media_url||'')}">${m.media_url?(m.media_type==='video'?`<video class="chat-media chat-video" src="${esc(m.media_url)}" controls playsinline preload="metadata"></video>`:`<button type="button" class="chat-media-open" data-media-open="${esc(m.media_url)}" aria-label="Şəkli böyüt"><img class="chat-media" src="${esc(m.media_url)}" alt=""></button>`):''}${m.body?`<p>${esc(m.body)}</p>`:''}<time>${timeText(m.created_at)}${m.edited_at?' · ✎':''}${m.sender_id===currentUser.id&&m.is_read?' ✓✓':''}</time>${m.sender_id===currentUser.id?`<div class="message-actions"><button type="button" data-edit-message="${m.id}" title="Redaktə"><i class="fa-solid fa-pen"></i></button><button type="button" data-delete-message="${m.id}" title="Sil"><i class="fa-regular fa-trash-can"></i></button></div>`:''}</div>`).join('');root.scrollTop=root.scrollHeight;
  ...
  function initPWA(){
    if('serviceWorker'in navigator){window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('service-worker.js?v=30',{updateViaCache:'none'});await reg.update();let reloading=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(reloading)return;reloading=true;window.location.reload()})}catch(e){console.warn(e)}})}; window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();deferredInstallPrompt=e;$('#installBanner')?.removeAttribute('hidden')});$('#installBtn')?.addEventListener('click',async()=>{if(!deferredInstallPrompt){toast('iPhone/iPad: Safari → Paylaş → Add to Home Screen.','info');return}deferredInstallPrompt.prompt();await deferredInstallPrompt.userChoice;deferredInstallPrompt=null;$('#installBanner')?.setAttribute('hidden','')});
  }
