...
(() => {
  ...
  async function openConversation(peer,listing=''){
    ...
    const {data:p}=await sb.from('users').select('id,name,surname,avatar_url,membership_tier,is_verified').eq('id',peer).maybeSingle();const peerName=$('#chatPeerName');if(peerName){peerName.innerHTML=`${p?.is_verified?'<span class="verified-mark verified-rosette av-verified-left message-verified" title="Təsdiqlənmiş hesab"><i class="fa-solid fa-check"></i></span>':''}<span>${esc([p?.name,p?.surname].filter(Boolean).join(' ')||staticText('İstifadəçi'))}</span>`};const peerSub=$('#chatPeerLink .muted.tiny');if(peerSub)peerSub.textContent=window.__avOnlineUsers?.has?.(peer)?'Onlayn':'Oflayn';$('#chatPeerAvatar').src=p?.avatar_url||'assets/img/brand/icon-192.png';const peerShell=$('#chatPeerAvatarShell');if(peerShell){peerShell.className=`chat-peer-avatar-shell avatar-shell ${p?.membership_tier||'free'}`;peerShell.querySelectorAll('.avatar-crown,.avatar-gem').forEach(x=>x.remove());if(p?.membership_tier==='vip')peerShell.insertAdjacentHTML('beforeend','<i class="fa-solid fa-crown avatar-crown"></i>');else if(p?.membership_tier==='premium')peerShell.insertAdjacentHTML('beforeend','<i class="fa-solid fa-gem avatar-gem"></i>')};const peerLink=$('#chatPeerLink');if(peerLink)peerLink.href=`user.html?id=${encodeURIComponent(peer)}`;
