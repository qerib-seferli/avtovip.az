...
(() => {
  ...
  async function renderThread(peer,listing=''){
    ...
    root.onclick=async e=>{const del=e.target.closest('[data-delete-message]'),edit=e.target.closest('[data-edit-message]');if(del){const ok=await uiDialog({title:'Mesajı sil',message:'Bu mesajı silmək istəyirsiniz?',confirmText:'Sil',cancelText:'Ləğv et',danger:true});if(!ok)return;const bubble=del.closest('.bubble');bubble?.classList.add('av-message-pending');const {error}=await sb.from('messages').update({deleted_at:new Date().toISOString(),body:'',media_url:null,media_type:null}).eq('id',del.dataset.deleteMessage).eq('sender_id',currentUser.id);if(error){bubble?.classList.remove('av-message-pending');toast(error.message,'error')}else{bubble?.remove();renderConversations(currentUser.id);updateMessageBadge()}}if(edit){const id=edit.dataset.editMessage,b=edit.closest('.bubble'),old=b.querySelector('p')?.textContent||'';const text=await uiDialog({title:'Mesajı redaktə et',message:'Mesajın yeni mətnini yazın:',input:true,inputValue:old,confirmText:'Yadda saxla',cancelText:'Ləğv et'});if(text===null)return;const value=text.trim();if(!value){toast('Mesaj boş ola bilməz.','error');return}const para=b.querySelector('p'),previous=para?.textContent||'';if(para)para.textContent=value;b.classList.add('av-message-pending');const {error}=await sb.from('messages').update({body:value,edited_at:new Date().toISOString()}).eq('id',id).eq('sender_id',currentUser.id);b.classList.remove('av-message-pending');if(error){if(para)para.textContent=previous;toast(error.message,'error')}else{const tm=b.querySelector('time');if(tm&&!tm.textContent.includes('✎'))tm.textContent=tm.textContent.replace(' ✓✓','')+' · ✎'+(tm.textContent.includes('✓✓')?' ✓✓':'');renderConversations(currentUser.id)}}};
