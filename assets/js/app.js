...
(() => {
  ...
  function initThemeLang(){
    ...
    if(langSel){const labels={az:'🇦🇿',en:'🇬🇧',ru:'🇷🇺',tr:'🇹🇷',ka:'🇬🇪'};langSel.innerHTML=Object.entries(labels).map(([v,l])=>`<option value="${v}">${l}</option>`).join('');langSel.value=lang;langSel.addEventListener('change',()=>{lang=langSel.value;localStorage.setItem('avtovip-lang',lang);document.documentElement.dataset.lang=lang;applyTranslations();window.dispatchEvent(new CustomEvent('avtovip:language',{detail:{lang}}));location.reload()})}
    applyTranslations();
  }
  ...
  function brandLogoCandidates(make){
    ...
    return [...new Set([local,snap].filter(Boolean))];
  }
  async function brandRail(container){
    ...
    for(const make of makes){const b=document.createElement('button');b.className='brand-chip';b.type='button';b.dataset.brand=make.name;const logos=brandLogoCandidates(make);const fallback=(make.name||'?').split(/\s+/).map(x=>x[0]).join('').slice(0,3).toUpperCase();b.innerHTML=`<span class="brand-logo-box">${logos.length?`<img loading="lazy" src="${esc(logos[0])}" alt="${esc(make.name)}">`:''}<b>${esc(fallback)}</b></span><span>${esc(make.name)}</span>`;const img=b.querySelector('img');if(img){img.addEventListener('error',()=>{img.remove();b.querySelector('.brand-logo-box')?.classList.add('fallback')},{once:true})}else b.querySelector('.brand-logo-box')?.classList.add('fallback');track.append(b)}
    container.classList.add('brands-scroll');let paused=false,last=performance.now();const speed=.025;
    const tick=now=>{if(!container.isConnected)return;const dt=Math.min(50,now-last);last=now;if(!paused&&container.scrollWidth>container.clientWidth+4){container.scrollLeft+=dt*speed;if(container.scrollLeft>=container.scrollWidth-container.clientWidth-2)container.scrollLeft=0}requestAnimationFrame(tick)};requestAnimationFrame(tick);
    ['pointerdown','touchstart','wheel'].forEach(ev=>container.addEventListener(ev,()=>paused=true,{passive:true}));['pointerup','touchend','pointercancel','mouseleave'].forEach(ev=>container.addEventListener(ev,()=>setTimeout(()=>paused=false,500),{passive:true}));
  }
  ...
  async function initProfile(){
    ...
    $('#profileName').innerHTML=`${flag?`<span class="profile-country-flag" title="${country}">${flag}</span> `:''}${esc([profile?.name,profile?.surname].filter(Boolean).join(' ')||user.email)}`;
    ...
    const nameBox=$('#profileName'); if(profile?.is_verified&&nameBox&&!nameBox.querySelector('.verified-mark'))nameBox.insertAdjacentHTML('beforeend',' <i class="fa-solid fa-circle-check verified-mark" title="Verified"></i>');
    ...
    if($('#walletBalance'))$('#walletBalance').textContent=money(profile?.wallet_balance||0,'AZN');
    $('#profileForm')?.addEventListener('submit',async e=>{e.preventDefault();const fd=new FormData(e.currentTarget);const payload={name:fd.get('name')?.trim(),surname:fd.get('surname')?.trim(),phone:fd.get('phone')?.trim(),whatsapp_phone:fd.get('whatsapp_phone')?.trim(),country_code:fd.get('country_code')||'AZ',city:fd.get('city')?.trim(),address:fd.get('address')?.trim(),bio:fd.get('bio')?.trim()};const {error}=await sb.from('users').update(payload).eq('id',user.id);if(error)toast(error.message,'error');else{toast('Profil yeniləndi.','success');loadCurrent()}});
    $('#walletTopupBtn')?.addEventListener('click',async()=>{const value=await uiDialog({title:'Balansı artır',message:'Balansa əlavə etmək istədiyiniz məbləği yazın (AZN):',input:true,confirmText:'Sorğu yarat'});const amount=Number(String(value||'').replace(',','.'));if(!Number.isFinite(amount)||amount<=0)return;const {error}=await sb.from('payment_requests').insert({user_id:user.id,target_type:'wallet_topup',plan_code:'wallet_topup',amount,currency:'AZN',payment_method:'payment_request',payer_note:''});if(error)toast(error.message,'error');else{toast('Balans artırma sorğusu yaradıldı.','success');loadOwnPayments(user.id)}});
  ...
  async function openConversation(peer,listing=''){
    ...
    const {data:p}=await sb.from('users').select('id,name,surname,avatar_url').eq('id',peer).maybeSingle();$('#chatPeerName').textContent=[p?.name,p?.surname].filter(Boolean).join(' ')||staticText('İstifadəçi');$('#chatPeerAvatar').src=p?.avatar_url||'assets/img/brand/icon-192.png';const peerLink=$('#chatPeerLink');if(peerLink)peerLink.href=`user.html?id=${encodeURIComponent(peer)}`;
    $('#chatBack')?.addEventListener('click',()=>$('#messagesShell').classList.remove('chat-selected'),{once:true});await renderThread(peer,listing);
    const form=$('#chatForm'),blocked=await blockedRelationship(peer),notice=$('#chatBlockedNotice');if(form)form.hidden=blocked;if(notice){notice.hidden=!blocked;notice.textContent=blocked?runtimeText('Bu istifadəçi ilə mesajlaşma mümkün deyil.'):''}
    if(blocked)return;
    form.onsubmit=async e=>{e.preventDefault();if(await blockedRelationship(peer)){form.hidden=true;if(notice){notice.hidden=false;notice.textContent=runtimeText('Bu istifadəçi ilə mesajlaşma mümkün deyil.')}return}const input=$('#chatInput');const body=input.value.trim();if(!body)return;input.value='';const {error}=await sb.from('messages').insert({listing_id:listing||null,sender_id:currentUser.id,receiver_id:peer,body});if(error){toast(error.message==='AVTOVIP_BLOCKED_MESSAGE'?runtimeText('Bu istifadəçi ilə mesajlaşma mümkün deyil.'):error.message,'error');input.value=body}else renderThread(peer,listing)};
  }
