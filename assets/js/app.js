...
(() => {
  ...
  async function openStory(s,p={}){
    ...
    modal.querySelector('#storyCommentFocus').onclick=()=>{holdStory();commentInput.focus()};commentInput.addEventListener('focus',holdStory);modal.querySelector('.story-card')?.addEventListener('click',e=>{if(modal.classList.contains('story-comments-open')&&!e.target.closest('.story-top')){e.preventDefault();closeComments()}});
    ...
    modal.querySelector('#storyCommentForm').onsubmit=async e=>{e.preventDefault();if(!currentUser){toast('Şərh yazmaq üçün giriş edin.','info');return}const body=commentInput.value.trim();if(!body)return;const {error}=await sb.from('story_comments').insert({story_id:s.id,user_id:currentUser.id,body});if(error)toast(error.message,'error');else{commentInput.value='';await loadSocial();commentsRoot.scrollTop=commentsRoot.scrollHeight;holdStory()}};
