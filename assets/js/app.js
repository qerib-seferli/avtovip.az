...
(() => {
  ...
  async function openStory(s,p={}){
    ...
    modal.querySelector('#storyCommentFocus').onclick=()=>{holdStory();commentInput.focus()};commentInput.addEventListener('focus',holdStory);modal.querySelector('.story-card')?.addEventListener('click',e=>{if(modal.classList.contains('story-comments-open')&&!e.target.closest('.story-top')){e.preventDefault();closeComments()}});modal.querySelector('.story-social')?.addEventListener('click',e=>{if(!modal.classList.contains('story-comments-open'))return;if(e.target.closest('.story-comments,.story-comment-form,.story-social-actions,.story-social-head'))return;e.preventDefault();closeComments()});
