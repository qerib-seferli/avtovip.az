...
(() => {
  ...
  async function openConversation(peer,listing=''){
    ...
    $('#chatBack')?.addEventListener('click',()=>{if(history.state?.avChat)history.back();else{const shell=$('#messagesShell');shell.classList.remove('chat-selected');pane.classList.remove('open');delete pane.dataset.peer;delete pane.dataset.listing;}},{once:true});
  ...
  function initPWA(){
    if('serviceWorker'in navigator){window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('service-worker.js?v=29',{updateViaCache:'none'});await reg.update();let reloading=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(reloading)return;reloading=true;window.location.reload()})}catch(e){console.warn(e)}})}; window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();deferredInstallPrompt=e;$('#installBanner')?.removeAttribute('hidden')});$('#installBtn')?.addEventListener('click',async()=>{if(!deferredInstallPrompt){toast('iPhone/iPad: Safari → Paylaş → Add to Home Screen.','info');return}deferredInstallPrompt.prompt();await deferredInstallPrompt.userChoice;deferredInstallPrompt=null;$('#installBanner')?.setAttribute('hidden','')});
  }
