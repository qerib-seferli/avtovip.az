...
(() => {
  ...
  async function openConversation(peer,listing=''){
    ...
    form.onsubmit=async e=>{e.preventDefault();const current=await blockedRelationship(peer);if(current.blocked){form.hidden=true;if(notice){notice.hidden=false;notice.textContent=current.byMe?runtimeText('Bu istifadəçini blokdan çıxarmadan mesaj yaza bilməzsiniz.'):runtimeText('Bu istifadəçi sizi bloklayıb. Mesaj göndərmək mümkün deyil.')}return}const input=$('#chatInput');const body=input.value.trim();if(!body)return;const {error}=await sb.from('messages').insert({listing_id:listing||null,sender_id:currentUser.id,receiver_id:peer,body});if(error){toast(error.message==='AVTOVIP_BLOCKED_MESSAGE'?runtimeText('Bu istifadəçi ilə mesajlaşma mümkün deyil.'):error.message,'error');return}input.value='';renderThread(peer,listing)};
  }
