...
(() => {
  ...
  async function openStory(s,p={},storyList=[],storyProfiles={}){
    ...
    if(emojiBtn&&emojiPicker){emojiPicker.innerHTML=storyEmojis.map(x=>`<button type="button" data-story-emoji="${x}">${x}</button>`).join('');emojiBtn.onclick=()=>{emojiPicker.hidden=!emojiPicker.hidden};emojiPicker.onclick=e=>{const b=e.target.closest('[data-story-emoji]');if(!b)return;const start=commentInput.selectionStart??commentInput.value.length,end=commentInput.selectionEnd??start;commentInput.value=commentInput.value.slice(0,start)+b.dataset.storyEmoji+commentInput.value.slice(end);const pos=start+b.dataset.storyEmoji.length;commentInput.focus();commentInput.setSelectionRange(pos,pos);emojiPicker.hidden=false}}
